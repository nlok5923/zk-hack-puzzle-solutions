...
pub fn main() {
    ...
    let pk_r = blob.rec_pk.clone();
    let x_g2 = blob.c.clone().hash_to_curve();

    for (i, msg) in messages.iter().enumerate() {
        let pkr_sk = (blob.c.1.clone() - msg.0).into_affine();

        let lhs = { Bls12_381::pairing(pk_r, blob.s.clone()) }; //  G1, G2
        let rhs = { Bls12_381::pairing(pkr_sk, x_g2) }; // G1, G2

        if lhs == rhs {
            println!("Message {} with index {} ", msg.0, i);
            break;
        }
    }
}
