...
use ark_std::{rand::{self, SeedableRng}, UniformRand, Zero};
use rand::Rng;
...
fn main() {
    ...
    // println!("Public keys: {:?}", public_keys[0]);
    ...
    let message = b"nlok5923";

    let old_aggregate_key = public_keys
    .iter()
    .fold(G1Projective::zero(), |acc, (pk, _)| acc + pk)
    .into_affine();

    let new_sk = Fr::from(2);
    let new_key_public = (G1Affine::generator().mul(new_sk) - old_aggregate_key).into_affine();
    let new_key_proving = (G1Affine::generator().mul(new_sk)).into_affine();
    let new_proof = pok_prove(new_sk, new_key_index);
    let aggregate_signature = bls_sign(new_sk, message);

    pok_verify(new_key_proving, new_key_index, new_proof);
    let aggregate_key = public_keys
        .iter()
        .fold(G1Projective::from(new_key_public), |acc, (pk, _)| acc + pk)
        .into_affine();
...
// suppose i have been given 10 public keys where each public key is g*sk1, g*sk2 .... g*skn

// and i also have a proof for all the public keys which proves that i know the secret key for all the public keys and the proofs are constructed as

// g2*sk1, g2*sk2 .... g2*skn

// Now i need to generate a new public private key pair (pk, sk) where pk = g*sk such that when i aggregate all the keys 